...
.pbid-group-title{margin:0;font-size:13px;font-weight:700;letter-spacing:.01em;color:var(--text-mute)}
...
.pbid-own-title{margin:0 0 4px;font-size:13px;font-weight:700;letter-spacing:.01em;color:var(--text-mute)}
...
export function mount(tool) {
  ...
  function buildRightPanel() {
    ...
    /* ‼️ ต้องสร้างหลังกลุ่มครบแล้วเหมือนกัน เพราะมันเดินหา [data-param] ในแผง
       และต้อง prepend ทีหลังช่องค้นหา เพื่อให้ลำดับบนลงล่างคือ ค้นหา แล้วค่อยสรุปจำนวนที่แก้ */
    dirty = dirtyMarks({
      scope: rightBody, defaults,
      values: () => paramValues,
      // คำใต้เลขกลางมาจากชุดข้อมูลที่เลือก ไม่ใช่ค่าที่ผู้ใช้ตั้งเอง ข้ามให้ตรงกับ statekit
      skip: ["centerCaption"],
  ...
  function rangeField(labelText, name, { min, max, step, fmt = (v) => String(v) }, hint) {
    ...
    return mark(field(labelText, el("div", { class: "pbid-range-row" }, [input, out]), hint), name);
  }
  ...
  function buildOwnPicker() {
    ...
    function loadSheet() {
      ...
      noteEl.textContent = tr(`อ่านได้ ${table.rows.length} แถว`, `${pl(table.rows.length, "row", "rows")} read`);
    }
