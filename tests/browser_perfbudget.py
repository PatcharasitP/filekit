...
#    ‼️ v173 ย้ายตัวค้นหา (search.js) กับคำค้นสำรอง (registry-keys.js) ไปดึงตอนชี้หรือโฟกัสช่องค้นหา
#       สูตรในเครื่อง 59,768 เหลือ 50,621 ข้อ ④ กันสองไฟล์นี้หลุดกลับมาหน้าแรกเหมือนไฟล์คำแปล
#       ถ้าเต็มอีก ของที่ยังโหลดโดยไม่มีใครแตะคือ offline.js (แถบท้ายหน้า 3,088) กับ loader.js (ใช้ตอนชี้ป้ายเครื่องมือ 5,223)
...
def is_banned(url):
    ...
    if TOOL_JS_RE.search(low):
        return "src/tools/*.js"
    if "registry-en.js" in low:
        return "คำแปลอังกฤษ (โหลดเฉพาะโหมดอังกฤษ)"
    if "registry-keys.js" in low or re.search(r"/src/search\.js(\?|$)", low):
        return "ตัวค้นหา (ดึงตอนชี้หรือโฟกัสช่องค้นหา)"
