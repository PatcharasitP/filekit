...
def sc_pdf_merge(pg, files):
    ...
    pg.wait_for_selector(".s2-side-res .result:visible, .results .result:visible", timeout=STEP_TIMEOUT)
    out = dl(pg, lambda: pg.locator(".s2-side-res .result button:visible, .results .result button:visible").first.click(), "pdf_merge_out.pdf")
    d = fitz.open(str(out))
...
def sc_pdf_to_images(pg, files):
    ...
    pg.wait_for_selector(".s2-side-res button:visible, .results button:visible", timeout=STEP_TIMEOUT)
    out = dl(pg, lambda: pg.locator(".s2-side-res button:visible, .results button:visible", has_text="ZIP").first.click(), "pdf_to_images_out.zip")
...
def sc_pdf_to_word(pg, files):
    ...
    pg.wait_for_selector(".s2-side-res .result:visible, .results .result:visible", timeout=STEP_TIMEOUT)
    out = dl(pg, lambda: pg.locator(".s2-side-res .result button:visible, .results .result button:visible").first.click(), "pdf_to_word_out.docx")
...
def sc_word_to_pdf(pg, files):
    ...
    pg.wait_for_selector(".s2-side-res .result:visible, .results .result:visible", timeout=STEP_TIMEOUT)
    out = dl(pg, lambda: pg.locator(".s2-side-res .result button:visible, .results .result button:visible").first.click(), "word_to_pdf_out.pdf")
    d = fitz.open(str(out))
    text = "".join(pg_.get_text() for pg_ in d)
...
def sc_excel_csv(pg, files):
    ...
    pg.wait_for_selector(".s2-side-res .result:visible, .results .result:visible", timeout=STEP_TIMEOUT)
    out = dl(pg, lambda: pg.locator(".s2-side-res .result button:visible, .results .result button:visible").first.click(), "excel_csv_out.csv")
    text = out.read_text(encoding="utf-8-sig")
