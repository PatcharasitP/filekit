  ...
  const ws = workspace(tool, {
    ...
    right: { title: tr("ตั้งรหัส", "Password"), node: el("div", {}, [
      ...
      el("div", { class: "pt-warn" },
         tr("ลืมรหัสแล้วกู้ไม่ได้เลย เพราะไฟล์ไม่เคยออกจากเครื่องคุณ ฟ้าและเว็บนี้ไม่มีสำเนารหัสไว้ที่ไหนทั้งสิ้น",
      ...
      el("details", { class: "pt-more" }, [
        ...
        el("div", {}, [
          ...
          el("div", { class: "pt-warn" },
             tr("สิทธิ์พวกนี้เป็นแค่คำขอที่ฝากไว้ในไฟล์ โปรแกรมอ่าน PDF บางตัวไม่ทำตาม ถ้าเนื้อหาห้ามหลุดจริง ๆ อย่าส่งไฟล์ออกไปเลยจะปลอดภัยกว่า",
                "These are only requests stored in the file. Some PDF readers ignore them. If the content truly must not get out, not sending the file at all is safer")),
          field(tr("รหัสเจ้าของเอกสาร", "Owner password"), owner,
                tr("ใช้สำหรับปลดข้อจำกัดข้างบน ถ้าไม่ใส่และมีการห้ามอะไรไว้ ฟ้าจะสุ่มรหัสที่ไม่มีใครรู้ให้ ข้อห้ามจะได้มีผลจริง",
                   "Used to lift the limits above. Leave it empty while something is restricted and a random password nobody knows is used instead, so the limits actually hold")),
        ]),
      ]),
    ]) },
