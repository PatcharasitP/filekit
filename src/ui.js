 ...
 */
/** แถวบอกคีย์ลัดของเครื่องมือ
 *
 * ‼️ พี่ปอนด์ทัก 20/09/2026 ว่า "ปุ่มพวกคีย์ลัดยังไม่เห็นนะ" ซึ่งจริง
 *   เว็บนี้มีคีย์ลัดใช้ได้จริงหลายตัว แต่ไม่เคยบอกใครเลยสักที่
 *   และเราห้าม tooltip ของเบราว์เซอร์ทั้งเว็บ (ไม่โผล่บนจอสัมผัส หน่วงครึ่งวินาทีบนเมาส์)
 *   จึงต้องเขียนไว้บนหน้าจอตรง ๆ ในที่ที่คนมองหาวิธีใช้อยู่แล้ว คือใต้พื้นที่ทำงาน
 *
 * ‼️ ซ่อนทั้งแถวบนอุปกรณ์ที่ไม่มีเมาส์ เพราะคนใช้จอสัมผัสไม่มีคีย์บอร์ดให้กด
 *   บอกไปก็เป็นแค่ตัวหนังสือที่กินที่เปล่า ๆ (กติกาอยู่ใน CSS .keyhints)
 * ‼️ แต่ละคู่สั้น ๆ ไม่ใช่ประโยคยาว เพราะกติกาโปรเจกต์จำกัดข้อความบนจอ 100 ตัวอักษรต่อก้อน
 */
export function keyHints(pairs) {
  return el("div", { class: "keyhints" }, [
    el("span", { class: "keyhints-t" }, tr("คีย์ลัด", "Shortcuts")),
    ...pairs.map(([key, what]) =>
      el("span", { class: "keyhint" }, [el("kbd", { class: "kbd" }, key), el("span", {}, what)])),
  ]);
}
