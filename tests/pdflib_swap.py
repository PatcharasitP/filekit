...
def c_compress(pg):
    # ‼️ ต้องใช้ไฟล์ที่สมจริง ไม่ใช่ไฟล์ 1 KB สองหน้า
    #    เครื่องมือนี้ตัดสินใจจาก "บันทึกใหม่แล้วเล็กลงเกิน 2% ไหม" ซึ่งกับไฟล์จิ๋วจะแกว่งมาก
    #    (วัดจริง 21/09: ไฟล์ 1,105 ไบต์ ไลบรารีเก่าได้ 84.8% ไลบรารีใหม่ได้ 107.7% คือบวมขึ้น
    #     แต่ไฟล์ 137 KB เก่าได้ 88.4% ใหม่ได้ 92.9% ผ่านเกณฑ์ทั้งคู่)
    #    ไฟล์ที่คนเอามาบีบจริงคือไฟล์ใหญ่ ทดสอบด้วยไฟล์เล็กจึงวัดผิดเป้า
    doc = fitz.open()
    for i in range(12):
        pp = doc.new_page(width=595, height=842)
        for y in range(60, 780, 24):
            pp.insert_text((50, y), f"page {i+1} line {y} compress regression fixture", fontname="helv", fontsize=11)
    src = FIX / "cp.pdf"; doc.save(str(src)); doc.close()
