...
def main():
    ...
    with sync_playwright() as pw:
        ...
        print("\n── ⑤ ช่องปรับขนาดต้องใช้งานได้จริง ไม่ใช่มีแต่ป้าย")
        # ‼️ จับบั๊กที่ตาเปล่ามองว่า "ไม่มีช่องนี้" มากกว่ามองว่าของพัง (เจอจริง 21/09/2026)
        #    แถบเลื่อนกว้าง 0 เพราะ element ตัวเดียวถูกใส่ไว้สองกลุ่ม กลุ่มหลังดึงไปจากกลุ่มแรก
        open_editor(pg, src)
        for m in ("sign", "image"):
            mode(pg, m)
            w = pg.evaluate("""() => { const e = [...document.querySelectorAll('input[type=range]')]
                .find(x => !x.closest('.pe-grp')?.hidden);
                return e ? Math.round(e.getBoundingClientRect().width) : 0; }""")
            ck(f"โหมด {m}: แถบปรับความกว้างต้องกว้างพอให้ลากได้ (>80px)", w > 80, True, f"กว้าง {w}px")
