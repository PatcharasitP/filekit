...
export function mount(tool) {
  ...
  const ws = workspace(tool, {
    ...
    // ‼️ บอกให้ครบว่าเสียอะไรบ้าง ไม่ใช่แค่เรื่องข้อความ — ทางวาดหน้าใหม่เป็นภาพ
    //    ช่องกรอกฟอร์ม ไฮไลต์ และโน้ตติดหน้าจึงหายไปทั้งหมดด้วย (ยิงจริงยืนยันแล้ว 09/09/2026)
    // ‼️ แก้ 22/09/2026: เดิมเขียนเหมือนทุกไฟล์ถูกวาดใหม่เป็นภาพ แต่ตั้งแต่มีทางบีบแบบเก็บข้อความ
    //    ไฟล์ที่มีข้อความจะไม่ถูกวาดใหม่เลยถ้าผู้ใช้ไม่ยอมเอง ข้อความเดิมจึงขัดกับผลที่ขึ้นในแผงผลลัพธ์
    //    (เห็นจากภาพจริง: ผลบอก "ข้อความยังค้นหาได้" แต่แถบล่างบอก "ข้อความจะค้นหาไม่ได้")
    note: tr("ไฟล์ที่มีข้อความจะบีบแบบเก็บข้อความไว้ ถ้าต้องวาดหน้าใหม่เป็นภาพ ซึ่งทำให้ข้อความกับฟอร์มหาย จะถามก่อน",
      "Files with text keep their text. If pages must be redrawn as images, which loses text and form fields, you are asked first"),
  });
