...
#      tests/runp.sh changed          (tests/pick.py เลือกจากไฟล์ที่แก้ ไฟล์กลางได้ชุดเต็ม แก้เฉพาะจุดได้ชุดที่เกี่ยว)
...
# ‼️ changed = ให้ tests/pick.py เลือกชุดจากไฟล์ที่แก้ (กติกาพี่ปอนด์เคาะ 26/09/2026)
#    เดิมรันชุดเต็มทุกครั้งที่ปล่อย 45 ถึง 60 นาที แม้แก้คำค้นคำเดียว วันนั้นชุดเต็ม 3 รอบไม่เจออะไรเพิ่ม
#    แก้ไฟล์กลางได้ชุดเต็ม แก้เฉพาะจุดได้เทส node ทั้งหมด + ชุดที่เกี่ยว ตัวเลือกพังหรือไม่แน่ใจ = ชุดเต็มเสมอ
if [ "${1:-}" = "changed" ]; then
  picked=$(python3 tests/pick.py --explain) || picked="all"
  [ -n "$picked" ] || { echo "ไม่มีไฟล์เปลี่ยน ไม่มีชุดที่ต้องรัน"; exit 0; }
  if [ "$picked" = "all" ]; then set -- all; else set -- $picked; fi
fi
