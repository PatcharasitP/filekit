...
def chain7_next_card_navigation(browser):
    ...
    try:
        ...
        # ไฟล์ที่ทำไว้ในเครื่องมือก่อนหน้าตามไปด้วยไหม — คาดว่า "ไม่" (ไม่มีกลไกส่งไฟล์ข้ามเครื่องมือ)
        carried = pg.locator(".file-row").count() > 0 or pg.locator(".results .result").count() > 0
        if not carried:
            limitations.append(
                f"ไฟล์/ผลลัพธ์จาก pdf-merge ไม่ตามไปที่ {next_id} อัตโนมัติ — ผู้ใช้ต้องดาวน์โหลดแล้วอัปโหลดเองใหม่ "
                f"(ไม่มี mechanism ส่งไฟล์ข้ามเครื่องมือใน src/*.js ตรวจแล้ว — ถือเป็นโอกาสปรับปรุง ไม่ใช่บั๊ก)")
        steps.append(("ตรวจว่าไฟล์ตามไปด้วยไหม", "-", "-",
                      "มีไฟล์/ผลลัพธ์ค้างอยู่ในเครื่องมือใหม่หรือไม่",
                      "ไฟล์ตามไปด้วย" if carried else "ไม่ตามไป (ข้อจำกัดที่รู้อยู่แล้ว)"))
